...
#       print_r($this->uri->segment(2));die();
      ...
      $strt = 1;
      $limit = 10;
      $filter_data = $this->callAPI('GET', 'https://api.monjin.com/f/22/1/1/list/published-job?filter=code['.$this->uri->segment(2).']', $token,$strt,$limit);
...
#      print_r($data);die();
   ...
   public function details_records(){
      $token = $this->get_access_token();
      $job_data = $this->callAPIFeed('GET', 'https://api.monjin.com/f/22/1/1/job/Feed', $token);
      $data['response'] = json_decode($job_data, true);
